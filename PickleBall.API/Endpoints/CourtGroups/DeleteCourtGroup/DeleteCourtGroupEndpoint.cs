...
namespace PickleBall.API.Endpoints.CourtGroups.DeleteCourtGroup
{
    ...
    public class DeleteCourtGroupEndpoint
        : EndpointBaseAsync.WithRequest<DeleteCourtGroupRequest>.WithActionResult
    {
        ...
        public override async Task<ActionResult> HandleAsync(
            DeleteCourtGroupRequest request,
            CancellationToken cancellationToken = default
        )
        {
            Result result = await _mediator.Send(
                new DeleteCourtGroupCommand { Id = request.Id },
                cancellationToken
            );
